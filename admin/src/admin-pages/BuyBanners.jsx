...
export default function BuyBanners() {
  ...
  const filteredBuyBanners = buyBanners.filter((b) => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await apiGetAllBuyBanners(token);
      setBuyBanners(res.data?.buyBanners || []);
    } catch {
      setBuyBanners([]);
    } finally {
      setLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await apiUpdateBuyBanner(editing._id, fd, token);
      } else {
        await apiCreateBuyBanner(fd, token);
      }
      setFormOpen(false);
      load();
    } catch {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Delete this buy banner?')) return;
    setDeletingId(id);
    try {
      await apiDeleteBuyBanner(id, token);
      setBuyBanners((p) => p.filter((b) => b._id !== id));
    } catch {
      alert('Delete failed');
    } finally {
      setDeletingId('');
  ...
  const handleToggle = async (id) => {
    setTogglingId(id);
    try {
      const res = await apiToggleBuyBannerStatus(id, token);
      setBuyBanners((p) =>
        p.map((b) => (b._id === id ? res.data.buyBanner : b)),
      );
    } catch {
      alert('Toggle failed');
    } finally {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      <div className="border border-gray-200 rounded-xl p-4">
          ...
          <div className="flex flex-1 border border-gray-200 rounded-lg overflow-hidden focus-within:border-orange-400">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search buy banners..."
        ...
        ) : filteredBuyBanners.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <p className="text-gray-400 text-lg">
              {search ? 'No matching buy banners' : 'No buy banners yet'}
            </p>
            <p className="text-gray-400 text-sm mt-1">
              {search
                ? 'Try a different search term'
                : 'Click "Add Buy Banner" to create your first banner'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {filteredBuyBanners.map((b) => (
              <div
                key={b._id}
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 pb-4 shrink-0 border-b border-gray-100">
              <h2 className="text-lg font-semibold text-gray-900">
                {editing ? 'Edit Buy Banner' : 'Add Buy Banner'}
              </h2>
            </div>
            ...
            >
              ...
              {/* <div>
                ...
                <input
                  ...
                  placeholder="Best deals on the products you want..."
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-orange-400"
                />
              </div> */}
