...
export default function RentBanners() {
  const [rentBanners, setRentBanners] = useState([]);
  ...
  const filteredRentBanners = rentBanners.filter((b) => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await apiGetAllRentBanners(token);
      setRentBanners(res.data?.rentBanners || []);
    } catch {
      setRentBanners([]);
    } finally {
      setLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await apiUpdateRentBanner(editing._id, fd, token);
      } else {
        await apiCreateRentBanner(fd, token);
      }
      setFormOpen(false);
      load();
    } catch {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Delete this rent banner?')) return;
    setDeletingId(id);
    try {
      await apiDeleteRentBanner(id, token);
      setRentBanners((p) => p.filter((b) => b._id !== id));
    } catch {
      alert('Delete failed');
    } finally {
      setDeletingId('');
  ...
  const handleToggle = async (id) => {
    setTogglingId(id);
    try {
      const res = await apiToggleRentBannerStatus(id, token);
      setRentBanners((p) =>
        p.map((b) => (b._id === id ? res.data.rentBanner : b)),
      );
    } catch {
      alert('Toggle failed');
    } finally {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      <div className="border border-gray-200 rounded-xl p-4">
          ...
          <div className="flex flex-1 border border-gray-200 rounded-lg overflow-hidden focus-within:border-orange-400">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search rent banners..."
        ...
        ) : filteredRentBanners.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <p className="text-gray-400 text-lg">
              {search ? 'No matching rent banners' : 'No rent banners yet'}
            </p>
            <p className="text-gray-400 text-sm mt-1">
              {search
                ? 'Try a different search term'
                : 'Click "Add Rent Banner" to create your first banner'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {filteredRentBanners.map((b) => (
              <div
                key={b._id}
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 pb-4 shrink-0 border-b border-gray-100">
              <h2 className="text-lg font-semibold text-gray-900">
                {editing ? 'Edit Rent Banner' : 'Add Rent Banner'}
              </h2>
            </div>
            ...
            >
              ...
              {/* <div>
                ...
                <input
                  ...
                  placeholder="Rent What You Need, When You Need It"
              ...
              {/* <div>
                ...
                <input
                  ...
                  placeholder="Affordable rentals near you..."
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-orange-400"
                />
              </div> */}
