...
export default function RentOffersPage() {
  ...
  const filteredRentOffers = rentOffers.filter((r) => {
    const q = search.toLowerCase();
    return (
      r.title?.toLowerCase().includes(q) ||
      r.subtitle?.toLowerCase().includes(q) ||
      r.clickableUrl?.toLowerCase().includes(q)
    );
  });
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await apiGetAllRentOffers(token);
      setRentOffers(res.data?.rentOffers || []);
    } catch {
      setRentOffers([]);
    } finally {
      setLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await apiUpdateRentOffer(editing._id, fd, token);
      } else {
        await apiCreateRentOffer(fd, token);
      }
      setFormOpen(false);
      load();
    } catch {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Delete this rent offer?')) return;
    setDeletingId(id);
    try {
      await apiDeleteRentOffer(id, token);
      setRentOffers((p) => p.filter((r) => r._id !== id));
    } catch {
      alert('Delete failed');
    } finally {
      setDeletingId('');
  ...
  const handleToggle = async (id) => {
    setTogglingId(id);
    try {
      const res = await apiToggleRentOfferStatus(id, token);
      setRentOffers((p) =>
        p.map((r) => (r._id === id ? res.data.rentOffer : r)),
      );
    } catch {
      alert('Toggle failed');
    } finally {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      <div className="border border-gray-200 rounded-xl p-4">
          ...
          <div className="flex flex-1 border border-gray-200 rounded-lg overflow-hidden focus-within:border-orange-400">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search rent offers..."
        ...
        ) : filteredRentOffers.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <p className="text-gray-400 text-lg">
              {search ? 'No matching rent offers' : 'No rent offers yet'}
            </p>
            <p className="text-gray-400 text-sm mt-1">
              {search
                ? 'Try a different search term'
                : 'Click "Add Rent Offer" to create your first rent offer'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {filteredRentOffers.map((r) => (
              <div
                key={r._id}
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 pb-4 shrink-0 border-b border-gray-100">
              <h2 className="text-lg font-semibold text-gray-900">
                {editing ? 'Edit Rent Offer' : 'Add Rent Offer'}
              </h2>
            </div>
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  When set, clicking the rent offer image will redirect to this
                  URL
                </p>
              </div>
