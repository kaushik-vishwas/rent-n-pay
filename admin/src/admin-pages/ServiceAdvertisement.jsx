...
export default function ServiceAdvertisementPage() {
  const [serviceAdvertisements, setServiceAdvertisements] = useState([]);
  ...
  const filteredServiceAdvertisements = serviceAdvertisements.filter((a) => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await apiGetAllServiceAdvertisements(token);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await apiUpdateServiceAdvertisement(editing._id, fd, token);
      } else {
        await apiCreateServiceAdvertisement(fd, token);
      }
      setFormOpen(false);
      load();
    } catch {
  ...
  const confirmDelete = async () => {
    ...
    setDeletingId(id);
    try {
      await apiDeleteServiceAdvertisement(id, token);
      setServiceAdvertisements((p) => p.filter((a) => a._id !== id));
    } catch {
      alert('Delete failed');
    } finally {
      setDeletingId('');
  ...
  const handleToggle = async (id) => {
    setTogglingId(id);
    try {
      const res = await apiToggleServiceAdvertisementStatus(id, token);
      setServiceAdvertisements((p) =>
        p.map((a) => (a._id === id ? res.data.serviceAdvertisement : a)),
      );
    } catch {
      alert('Toggle failed');
    } finally {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      <div className="border border-gray-200 rounded-xl p-4">
        ...
        {loading ? (
        ...
        ) : filteredServiceAdvertisements.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {filteredServiceAdvertisements.map((a) => (
              <div
                key={a._id}
