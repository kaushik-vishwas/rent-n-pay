...
//     setLoading(true);
//     try {
//       const res = await apiGetAllBanners(token);
...
//       if (editing) {
//         await apiUpdateBanner(editing._id, fd, token);
//       } else {
//         await apiCreateBanner(fd, token);
//       }
//       setFormOpen(false);
//       load();
//     } catch {
...
//     setDeletingId(id);
//     try {
//       await apiDeleteBanner(id, token);
...
//     setTogglingId(id);
//     try {
//       const res = await apiToggleBannerStatus(id, token);
...
//               {search ? 'No matching banners' : 'No banners yet'}
//             </p>
...
//                   placeholder="Buy, Rent, Or Book Services"
...
//                   }
//                   placeholder="The hyper-local marketplace..."
//                   className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-orange-400"
//                 />
//               </div> */}
...
import {
  ...
  apiDeleteBanner,
  apiToggleBannerStatus,
  apiGetAllRentBanners,
  apiCreateRentBanner,
  apiUpdateRentBanner,
  apiDeleteRentBanner,
  apiToggleRentBannerStatus,
  apiGetAllBuyBanners,
  apiCreateBuyBanner,
  apiUpdateBuyBanner,
  apiDeleteBuyBanner,
  apiToggleBuyBannerStatus,
} from '@/service/api';
...
const TAB_CONFIG = {
  home: {
    label: 'Home',
    listKey: 'banners',
    getAll: apiGetAllBanners,
    create: apiCreateBanner,
    update: apiUpdateBanner,
    remove: apiDeleteBanner,
    toggle: apiToggleBannerStatus,
  ...
  rent: {
    label: 'Rent',
    listKey: 'rentBanners',
    getAll: apiGetAllRentBanners,
    create: apiCreateRentBanner,
    update: apiUpdateRentBanner,
    remove: apiDeleteRentBanner,
    toggle: apiToggleRentBannerStatus,
    toggleResponseKey: 'rentBanner',
    addButtonLabel: '+ Add Rent Banner',
    searchPlaceholder: 'Search rent banners...',
    emptyLabel: 'rent banners',
    activeLabel: 'rent page',
  },
  buy: {
    label: 'Buy',
    listKey: 'buyBanners',
    getAll: apiGetAllBuyBanners,
    create: apiCreateBuyBanner,
    update: apiUpdateBuyBanner,
    remove: apiDeleteBuyBanner,
    toggle: apiToggleBuyBannerStatus,
    toggleResponseKey: 'buyBanner',
    addButtonLabel: '+ Add Buy Banner',
    searchPlaceholder: 'Search buy banners...',
    emptyLabel: 'buy banners',
...
export default function BannersPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setBanners(res.data?.[cfg.listKey] || []);
    } catch {
      setBanners([]);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id) => {
    if (!confirm(`Delete this ${cfg.label.toLowerCase()} banner?`)) return;
    setDeletingId(id);
    try {
  ...
  const handleToggle = async (id) => {
    ...
    try {
      ...
      setBanners((p) =>
        p.map((b) => (b._id === id ? res.data[cfg.toggleResponseKey] : b)),
      );
    } catch {
      alert('Toggle failed');
    } finally {
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-gray-400 text-sm mt-1">
              {search
                ? 'Try a different search term'
                : `Click "${cfg.addButtonLabel.replace('+ ', '')}" to create your first banner`}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl flex flex-col max-h-[90vh]">
            <div className="p-6 pb-4 shrink-0 border-b border-gray-100">
              <h2 className="text-lg font-semibold text-gray-900">
                {editing
                  ? `Edit ${cfg.label} Banner`
                  : `Add ${cfg.label} Banner`}
              </h2>
            </div>
