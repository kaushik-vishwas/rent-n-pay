...
export default function BuyAdvertisementPage() {
  ...
  const filteredBuyAdvertisements = buyAdvertisements.filter((a) => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await apiGetAllBuyAdvertisements(token);
      setBuyAdvertisements(res.data?.buyAdvertisements || []);
    } catch {
      setBuyAdvertisements([]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await apiUpdateBuyAdvertisement(editing._id, fd, token);
      } else {
        await apiCreateBuyAdvertisement(fd, token);
      }
      setFormOpen(false);
      load();
    } catch {
  ...
  const confirmDelete = async () => {
    ...
    setDeletingId(id);
    try {
      await apiDeleteBuyAdvertisement(id, token);
      setBuyAdvertisements((p) => p.filter((a) => a._id !== id));
    } catch {
      alert('Delete failed');
    } finally {
      setDeletingId('');
  ...
  const handleToggle = async (id) => {
    setTogglingId(id);
    try {
      const res = await apiToggleBuyAdvertisementStatus(id, token);
      setBuyAdvertisements((p) =>
        p.map((a) => (a._id === id ? res.data.buyAdvertisement : a)),
      );
    } catch {
      alert('Toggle failed');
    } finally {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      <div className="border border-gray-200 rounded-xl p-4">
        ...
        {loading ? (
        ...
        ) : filteredBuyAdvertisements.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {filteredBuyAdvertisements.map((a) => (
              <div
                key={a._id}
