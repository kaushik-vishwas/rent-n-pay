...
function resolveProductVariant(product, line) {
  ...
  // Custom listings often omit variantId on order lines — use the sole SKU.
...
/** Vendor configuration price when admin/vendor actually set one. 0 = not set. */
/** Line deposit snapshot, else product (per unit × qty). Matches checkout display. */
...
export function vendorFacingTierAmount(tier, commissionRatePercent) {
  const vendorRent = positiveAmount(tier?.vendorRent);
  if (vendorRent) return vendorRent;
  const customer = positiveAmount(tier?.customerRent || tier?.pricePerDay);
  ...
  return Math.max(0, Math.round(customer - (customer * safeRate) / 100));
}
...
export function resolveVendorOrderLineDisplayAmount(
  ...
  // Table / order total = vendor product payout + refundable deposit.
  return computeVendorLineMoney(line, categoryRateMap, order).payout;
}

export function resolveVendorOrderLineProductAmount(
  line,
  categoryRateMap = {},
  order = null,
) {
  const qty = Math.max(1, Number(line?.quantity || 1));
  if (hasSnapshottedVendorUnitRate(line)) {
    const unit = Number(line.vendorUnitRateAtOrder);
    if (unit > 0) return unit * qty;
  }
  return computeVendorLineMoney(line, categoryRateMap, order).netProduct;
}
...
export function computeVendorLineMoney(line, categoryRateMap = {}, order = null) {
  ...
  // Option 2: frozen at checkout; legacy orders use commission path only.
  const configuredRate = hasSnapshottedVendorUnitRate(line)
    ? positiveAmount(Number(line.vendorUnitRateAtOrder))
    : 0;

  // Case 1 / 3: vendor configuration (or vendor's own listing price) is set
  // → show that amount as-is. No GST, care tax, or commission cut.
  if (configuredRate > 0) {
  ...
  // Case 2: vendor configuration empty → customer price minus commission + deposit
