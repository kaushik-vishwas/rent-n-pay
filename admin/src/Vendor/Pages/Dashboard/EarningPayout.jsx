...
//   );
//   y += 7;
//   pdf.text(`Vendor ID: ${s.vendorId || vendorUser?._id || '—'}`, 14, y);
...
//   pdf.text(formatINR(row.totalSales), 178, y, { align: 'right' });

//   y += 8;
//   pdf.text('Platform Fee (10%)', 18, y);
//   pdf.setTextColor(200, 60, 60);
//   pdf.text(`- ${formatINR(row.platformFee)}`, 178, y, { align: 'right' });
//   pdf.setTextColor(0);
...
//   pdf.text(formatINR(row.netPayout), 178, y, { align: 'right' });
//   pdf.setTextColor(0);
//   pdf.setFont(undefined, 'normal');
...
//     if (!bankForm.chequeImage && !bankDetails?.chequeImage) {
//       errors.chequeImage = 'Please upload a cancelled cheque / passbook.';
//     } else if (bankForm.chequeImage) {
...
//     1,
//     Math.ceil(filteredSettlements.length / PAGE_SIZE),
//   );
...
//                 </p>
//               </div>
//               <div className="relative" data-period-filter-menu>
//                 <button
//                   type="button"
...
//                   <svg
//                     className={`h-3 w-3 transition-transform ${periodMenuOpen ? 'rotate-180' : ''}`}
//                     viewBox="0 0 24 24"
//                     fill="none"
//                     stroke="currentColor"
//                     strokeWidth="2"
...
//                   <div
//                     role="menu"
//                     className="absolute right-0 top-full z-20 mt-1 min-w-[10rem] rounded-xl border border-gray-200 bg-white py-1 shadow-lg"
//                   >
...
//                             <button
//                               type="button"
//                               onClick={() => downloadSettlementPDF(row, user)}
...
//                     <div className="mt-4 rounded-lg bg-rose-500/10 border border-rose-400/30 px-3 py-2 text-xs text-rose-200">
//                       Rejected: {bankDetails.rejectionReason}
//                     </div>
//                   ) : null}
//                 </div>
//               ) : (
...
//                 disabled={otpSending}
//                 className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border-2 border-[#F97316] py-3 text-sm font-medium text-[#F97316] disabled:opacity-60"
//               >
//                 {otpSending ? (
//                   'Sending OTP...'
//                 ) : (
...
function downloadPeriodPDF(group, vendorUser) {
  ...
  pdf.text('Order', 18, y);
  pdf.text('Total Sales', 100, y);
  pdf.text('Platform Fee', 135, y);
