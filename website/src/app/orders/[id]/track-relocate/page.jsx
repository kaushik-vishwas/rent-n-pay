...
// Two-stage stepper: Requested -> Confirmed
function RelocationStepper({ isConfirmed }) {
  const labels = ['Requested', 'Confirmed'];
  const stepComplete = [true, isConfirmed];
      ...
      <div className="flex items-start min-w-[220px] sm:min-w-0">
        {labels.flatMap((label, i) => {
          ...
          const circleClass = done
            ...
            : 'bg-[#EEF2FF] border-indigo-400 text-indigo-500';
            ...
            >
              ...
              <p
                className={`mt-2 text-[10px] sm:text-xs font-medium text-center leading-tight px-0.5 ${
                  done ? 'text-gray-800' : 'text-indigo-600'
                }`}
              >
                {label}
              </p>
          ...
          if (i < labels.length - 1) {
            chunks.push(
              ...
              >
                <div
                  className={`h-full rounded-full ${isConfirmed ? 'w-full bg-emerald-500' : 'w-0'}`}
                />
              </div>,
            );
          }
...
export default function RelocateOrderTrackPage() {
  ...
  const relocationRequest = selectedLine?.relocationRequest || null;
  const isConfirmed = String(relocationRequest?.status || '') === 'confirmed';
  const oldAddr = relocationRequest?.oldAddressSnapshot || {
    name: order?.name,
    address: order?.address,
    phone: order?.phone,
  };
  const newAddr = relocationRequest?.newAddress || {};
    ...
    <div className="min-h-screen bg-[#F4F6FB] py-8 px-4 sm:px-6 pb-16">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mt-8 bg-white rounded-xl border border-gray-200 shadow-sm p-5 sm:p-6">
          <h2 className="text-base font-bold text-black">Relocation Status</h2>
          <div className="mt-6">
            <RelocationStepper isConfirmed={isConfirmed} />
          </div>
          <div
          ...
          >
            ...
            <p
              className={`text-xs sm:text-sm font-semibold whitespace-nowrap sm:whitespace-normal ${
                isConfirmed ? 'text-emerald-700' : 'text-indigo-700'
              }`}
            >
              {isConfirmed
                ? 'Relocation confirmed'
                : "We've received your relocation request."}
            </p>
          </div>
        </div>

        <div className="mt-5 bg-white rounded-xl border border-gray-200 shadow-sm p-5 sm:p-6">
          <h2 className="text-base font-bold text-black">Address Details</h2>
          ...
          <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="bg-white shadow-md rounded-xl p-4 sm:p-5">
              <div className="flex items-center gap-2 text-sm font-semibold text-black">
                ...
                {isConfirmed ? 'Old Address' : 'Current Address'}
              </div>
              <p className="mt-3 text-sm text-gray-700 leading-relaxed">
                {oldAddr?.name}
                <br />
                {String(oldAddr?.address || '')
                  .split(/[\n,]+/)
                  .map((s) => s.trim())
                  .filter(Boolean)
                  .join(', ') || oldAddr?.address}
              </p>
              {oldAddr?.phone ? (
        ...
        <div className="mt-5 bg-white rounded-xl border border-gray-200 shadow-sm p-5 sm:p-6">
          <h2 className="text-base font-bold text-black">Item</h2>
