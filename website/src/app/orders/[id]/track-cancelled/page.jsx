...
// Ordered step green (it did happen), final step red "Cancelled".
function CancelledShipmentStepper() {
  const labels = ['Ordered', 'Cancelled'];

  return (
    <div className="w-full overflow-x-auto pb-1">
      <div className="flex items-start min-w-[220px] sm:min-w-0">
        {labels.map((label, i) => {
          const isCancelStep = i === labels.length - 1;
          const circleClass = isCancelStep
            ...
            >
              ...
              <p
                className={`mt-2 text-[10px] sm:text-xs font-medium text-center leading-tight px-0.5 ${
                  isCancelStep ? 'text-red-600' : 'text-gray-800'
                }`}
              >
                {label}
              </p>
...
function rentalProductLines(order) {
  return (order.products || []).filter((l) => {
    const t = String(l?.productType || '').toLowerCase();
    if (t === 'sell') return false;
    const p = l?.product;
...
function orderReturnLine(order) {
  return (order.products || []).find((l) => {
    const s = String(l?.returnRequest?.status || '');
...
export default function CancelledOrderTrackPage() {
  ...
  const rrLine = useMemo(
    () => selectedLine ?? (order ? orderReturnLine(order) : null),
    [selectedLine, order],
  );
  ...
  const refundShown = hasReturnTracking
    ...
    : Number(rrLine?.refundBreakdown?.refundAmount ?? grandTotal);
    ...
    <div className="min-h-screen bg-[#F4F6FB] py-8 px-4 sm:px-6 pb-16">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mt-8 bg-white rounded-xl border border-gray-200 shadow-sm p-5 sm:p-6">
          <h2 className="text-base font-bold text-black">Order Status</h2>
          <div className="mt-6">
            <CancelledShipmentStepper />
          </div>
          <div className="mt-6 inline-flex w-auto max-w-full items-center gap-1 rounded-lg bg-red-50 border-2 border-red-200 px-3 sm:px-4 py-2.5 sm:py-3 text-sm text-black">
            <XCircle className="w-4 h-4 shrink-0 text-red-600" />
            <p className="text-xs sm:text-sm font-semibold text-red-700 whitespace-nowrap sm:whitespace-normal">
              Cancelled on {cancelledDetailDate}
              {rrLine?.cancelledBy === 'vendor' ? ' by vendor' : ''}
            </p>
          </div>
          ...
          {/* <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 px-3 sm:px-4 py-2.5 sm:py-3 flex items-center gap-2">
