...
// All steps complete (green) for a completed rental.
function CompletedShipmentStepper() {
  const labels = ['Ordered', 'Packed', 'In Transit', 'Completed'];
      ...
      <div className="flex items-start min-w-[280px] sm:min-w-0">
        {labels.flatMap((label, i) => {
          const chunks = [
            ...
            >
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center shrink-0 border-2 bg-emerald-500 border-emerald-500 text-white">
                <CheckCircle
                  className="w-5 h-5 sm:w-6 sm:h-6"
                  strokeWidth={2.5}
                />
              </div>
              <p className="mt-2 text-[10px] sm:text-xs font-medium text-center leading-tight px-0.5 text-gray-800">
                {label}
              </p>
            </div>,
          ];
          if (i < labels.length - 1) {
            chunks.push(
              ...
              >
                <div className="h-full rounded-full w-full bg-emerald-500" />
              </div>,
            );
          }
          return chunks;
...
export default function CompletedOrderTrackPage() {
    ...
    <div className="min-h-screen bg-[#F4F6FB] py-8 px-4 sm:px-6 pb-16">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mt-8 bg-white rounded-xl border border-gray-200 shadow-sm p-5 sm:p-6">
          ...
          <div className="mt-6">
            <CompletedShipmentStepper />
          </div>
          <div className="mt-6 inline-flex w-auto max-w-full items-center gap-1 rounded-lg bg-[#F0FDF4] border-2 border-[#B9F8CF] px-3 sm:px-4 py-2.5 sm:py-3 text-sm text-black">
            <CheckCircle2 className="w-4 h-4 shrink-0 text-emerald-600" />
            <p className="text-xs sm:text-sm font-semibold text-emerald-700 whitespace-nowrap sm:whitespace-normal">
              Thank you — rental closed
              {order.updatedAt ? ` on ${formatOrderDate(order.updatedAt)}` : ''}
            </p>
          </div>
        </div>
