...
// import { getProductDeliveryEtaLabel } from '@/lib/rentalPriceDisplay';
...
// // }
// function expectedDeliveryDate(order, overrideProduct) {
//   const product = overrideProduct ?? primaryProduct(order);
...
//     if (timelineUnit === 'hours') {
//       d.setHours(d.getHours() + timelineValue);
//     } else {
//       d.setDate(d.getDate() + timelineValue);
//     }
//   } else {
...
// //     return {
// //       pct: 0,
// //       message: 'You can cancel for a full refund at this stage.',
// //     };
...
// // function trackingBlurb(order, overrideProduct) {
// //   const st = normalizeStatus(order.status);
// //   const eta = expectedDeliveryDate(order, overrideProduct);
// //   if (st === 'shipped') {
...
//     return (
//       <>
//         Expected delivery by <strong>{eta}</strong>.
//       </>
//     );
//   }
//   if (st === 'confirmed') {
...
//     return typeof selectedLine.product === 'object'
...
//   // const blurb = useMemo(() => {
//   //   if (!order) return '';
//   //   return trackingBlurb(order, product);
//   // }, [order, product]);
//   const blurb = useMemo(() => {
//     if (!order) return '';
//     // Use the line's own status if available so the blurb matches this specific item
//     const lineStatus = selectedLine?.lineStatus || order.status;
//     return trackingBlurb({ ...order, status: lineStatus }, product);
//   }, [order, selectedLine, product]);
...
//     return price * qty;
//   }, [selectedLine, order]);
//   // const grandTotal = linePrice;
//   const grandTotal = useMemo(() => {
//     if (!selectedLine) return order ? orderGrandTotal(order) : 0;
//     // Per-product price only — pricePerDay × quantity
...
//   // Security/refundable deposit for this specific line (rentals only)
...
//   // refundable deposit (that's handled separately above). Only fields
//   // that actually have a value on this order contribute to the total.
...
//   // Cancellation fee applies to product price + deposit combined
//   // const cancellableBase = grandTotal + depositAmount;

//   // const feeAmount =
//   //   policy && cancellableBase > 0
//   //     ? Math.round((cancellableBase * policy.pct) / 100)
//   //     : 0;
//   // const refundAmount = Math.max(0, cancellableBase - feeAmount);
//   // Cancellation fee applies to product price ONLY — deposit is always
//   // refunded in full, untouched by the cancellation fee percentage.
//   const feeAmount =
...
//   //   const msg =
//   //     policy.pct > 0
//   //       ? `Cancel this item? A ${policy.pct}% fee applies. Estimated refund: ₹${formatMoney(refundAmount)}.`
//   //       : 'Cancel this item for a full refund?';
//   //   if (!window.confirm(msg)) return;
//   //   setCancelling(true);

//   //   // Pass the specific line's _id so only that line gets cancelled
//   //   const lineId = selectedLine?._id;

//   //   apiCancelMyOrder(order._id, lineId) // ← pass lineId
//   //     .then(() => {
...
//             <p className="text-lg font-bold text-black">{displayRef}</p>
...
//             <h1 className="text-2xl sm:text-3xl font-bold text-black tracking-tight mt-4">
//               Order #{displayRef}
//             </h1>
...
//             {order.vendorFulfillment?.delivery?.driverName ||
//             order.vendorFulfillment?.delivery?.driverPhone ||
//             order.vendorFulfillment?.delivery?.vehicleNumber ? (
//               <div className="bg-white shadow-md rounded-xl p-4 sm:p-5">
//                 <div className="flex items-center gap-2 text-sm font-semibold text-black">
//                   <div className="bg-[#EFF6FF] p-1.5 rounded-lg">
